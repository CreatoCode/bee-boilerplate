...
func (m *WxConfig) GetWxAccessToken(code string) (accessToken *WxAccessToken, err error) {
	...
	body, err := httpClient.Get(AccessTokenURL, []byte(params.Encode()))
	if err != nil {
		return accessToken, err
	}
...
func (m *WxAccessToken) GetUserInfo() (wxUserInfo *WxUserInfo, err error) {
	...
	body, err := httpClient.Get(UserInfoURL, []byte(params.Encode()))
	if err != nil {
		return nil, err
	}
...
func (m *WxAccessToken) GetRefreshToken(appid string) error {
	...
	if appid == "" {
		return errors.New(ErrAppIDEmpty)
	}

	if m.RefreshToken == "" {
		return errors.New(ErrRefreshTokenEmpty)
	}

	if m.OpenID == "" {
		return errors.New(ErrOpenIDEmpty)
	}
	...
	body, err := httpClient.Get(RefreshTokenURL, []byte(params.Encode()))
	if err != nil {
		return err
	}
...
func (m *WxAccessToken) CheckAccessToken() (ok bool, err error) {
	...
	if m.AccessToken == "" {
		return ok, errors.New(ErrAccessTokenEmpty)
	}

	if m.OpenID == "" {
		return ok, errors.New(ErrOpenIDEmpty)
	}
	...
	body, err := httpClient.Get(CheckAccessTokenURL, []byte(params.Encode()))
	if err != nil {
		return ok, err
	}
...
func (m *WxConfig) GetJsCode2Session(code string) (wXBizDataCrypt *WXBizDataCrypt, err error) {
	...
	body, err := httpClient.Get(JsCode2SessionURL, []byte(params.Encode()))
	if err != nil {
		return wXBizDataCrypt, err
	}
...
func (m *WXBizDataCrypt) WeDecryptData(encryptedData, iv string) (wechatEncryptedData *WechatEncryptedData, err error) {
	...
	resBytes = crypto.PKCS7UnPadding(resBytes)
